...
export const StatCostsBrief: Component<{ costs: Item["costs"] }> = (props) => (
  <div>
    <div class="flex items-center gap-4 mt-1">
      {["food", "wood", "gold", "stone", "oliveoil", "time", "popcap", "vizier"].map(
        (type) =>
...
export const StatIconNumber: Component<{
  label: string;
  icon?: string;
  item: UnifiedItem<Item>;
  stat: Stat;
  ...
  createEffect(
    on([globalAgeFilter, globalCivFilter, () => props.age?.()], () => {
      const { parts, ...rest } = calculateStatParts(props.stat, props.age?.() ?? globalAgeFilter(), { decimals: 3, item: props.item });
      setParts(parts.map(([v, ...p]) => [v * multiplier(), ...p]));
      setValues(Object.fromEntries(Object.entries(rest).map(([k, v]) => [k, v * multiplier()])) as any);
    })
  );
